...
class VM {
  ...
  constructor(compiled_program, ticks_limit) {
    ...
    this.ticks =
      ticks_limit === undefined
        ? new TicksLimit(-1)
        : new TicksLimit(ticks_limit);
    this.output_buffer = "";
  }
  /**
   * Runs the program.
    ...
    while (this.pc < this.bytecode.length && this.ticks.hasMoreTicks()) {
      ...
      switch (op.op) {
        ...
        case "READ":
          throw {
            name: "RuntimeError",
            message: "Not supported in this VM",
          };
        case "ifjump":
        ...
        default:
          throw {
            name: "RuntimeError",
            message: "Should never get here :( unkown byte code",
          };
      }
    }
  }
